...
* Defining & Referencing a Varibale;
...
* Creating or defining a macro Variale;
...
* All these varibles created get stored in Global Symbol Table;
...
* let us reference and print the above defined MV;
%PUT &city;
%PUT &dob;
...
* Below statement will take the whole statement as a big text and display it as it is;
...
* In case of apostaphy, double quoted saves the format of display;
...
%LET x = varlist;
* below will trough error, there should be MV declared before assignment;
...
* 1. SYMBOLGEN;
* Validating the Macro varibles in symbol table - use SYMBOLGEN;
* SYMBOLGEN is primarily used to debug and monitor the value a MV carries;
* It is usually specified as a OPTION command;
OPTION SYMBOLGEN;
...
%PUT _ALL_; *Displays all Automatic as well user defined MV from sysmbol table;
...
%PUT %SYSFUNC(left(%QSYSFUNC(TODAY(), weekdate.))); * almost similar to %SYSFUNC but 
